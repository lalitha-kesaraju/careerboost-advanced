        ...
        {!isActive && !feedback && (
          <div className="flex gap-4">
             <input 
              ...
              className="px-6 py-3 bg-white border border-gray-100 rounded-2xl text-sm min-w-[300px] shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-100"
             />
             <button 
              onClick={startSession}
              disabled={!role}
              className="bg-indigo-600 text-white px-8 py-3 rounded-2xl font-bold flex items-center gap-2 hover:bg-indigo-700 transition-all disabled:opacity-50"
             >
      ...
      <div className="flex-1 min-h-[500px] bg-white rounded-[3rem] border border-gray-100 shadow-sm relative overflow-hidden flex flex-col">
        {!isActive && !feedback && (
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-10 space-y-6">
             <div className="w-20 h-20 bg-indigo-50 text-indigo-600 rounded-3xl flex items-center justify-center animate-pulse">
                ...
                {transcript.map((msg, i) => (
                   <div key={i} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                      <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 ${
                        msg.role === 'user' ? 'bg-gray-100 text-gray-400' : 'bg-indigo-600 text-white shadow-lg shadow-indigo-100'
                      }`}>
                         {msg.role === 'user' ? <UserIcon className="w-4 h-4" /> : <Bot className="w-4 h-4" />}
                      </div>
                      <div className={`p-5 rounded-[1.5rem] max-w-[70%] text-sm ${
                        msg.role === 'user' ? 'bg-indigo-50 text-indigo-900 rounded-tr-none' : 'bg-gray-50 text-gray-700 rounded-tl-none italic serif'
                      }`}>
                         {msg.content}
                      </div>
                   </div>
        ...
        {feedback && !isActive && (
          <div className="absolute inset-0 flex flex-col items-center justify-center p-20 text-center bg-indigo-900 text-white">
                ...
                <div className="max-w-xl mx-auto space-y-6">
                   ...
                   <div className="pt-10 flex flex-col md:flex-row items-center justify-center gap-6">
                      <button 
                        ...
                        className="bg-white text-indigo-900 px-8 py-4 rounded-2xl font-bold flex items-center gap-2 hover:scale-105 transition-all text-sm uppercase tracking-widest"
                      >
                         New Session <ArrowRight className="w-4 h-4" />
                      </button>
                      <span className="text-xs text-indigo-300 font-mono uppercase tracking-tighter">Usage Recorded: 1 Session</span>
                   </div>
                </div>
             </motion.div>
          </div>
