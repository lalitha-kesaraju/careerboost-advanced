    ...
    <div className="space-y-12 pb-32">
       ...
       <div className="flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-6">
             <div className="w-16 h-16 bg-gray-900 rounded-[2rem] flex items-center justify-center text-cyan-400 shadow-2xl relative overflow-hidden group">
                <Bot className="w-8 h-8 relative z-10" />
                <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/20 to-transparent group-hover:scale-150 transition-transform duration-700" />
             </div>
             <div>
       ...
       <AnimatePresence mode="wait">
          {activeTab === 'insights' ? (
             ...
             >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                   {summaryData.map((item, i) => (
                      <motion.div 
                        ...
                        className="bg-white border border-gray-100 rounded-[3rem] p-10 shadow-xl shadow-gray-200/20 hover:border-cyan-200 transition-all group"
                      >
                ...
                {/* Deep Insight Card */}
                <div className="bg-gray-900 rounded-[4rem] p-16 text-white relative overflow-hidden shadow-2xl shadow-cyan-900/40">
                   <div className="absolute top-0 right-0 w-96 h-96 bg-cyan-500/10 rounded-full blur-[120px] -mr-20 -mt-20" />
                   <div className="absolute bottom-0 left-0 w-64 h-64 bg-purple-500/10 rounded-full blur-[100px] -ml-20 -mb-20" />
                   
                   <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                      <div className="space-y-8">
                         <div className="inline-flex items-center gap-3 px-4 py-2 bg-white/5 border border-white/10 rounded-full">
                            <Sparkles className="w-4 h-4 text-cyan-400" />
                            <span className="text-[10px] font-black uppercase tracking-widest text-cyan-100">Live Executive Summary</span>
                         </div>
                         <h3 className="text-5xl font-black tracking-tight leading-[0.95]">
                            Your System is <br/> 
                            <span className="text-cyan-400">74% Calibrated</span>
                         </h3>
                         ...
                         <div className="flex gap-4">
                            <button 
                              onClick={() => setActiveTab('chat')}
                              className="px-10 py-5 bg-cyan-600 hover:bg-cyan-700 text-white rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl shadow-cyan-900/20 transition-all transform active:scale-95 flex items-center gap-3"
                            >
                 ...
                 <div className="space-y-6">
                    {userData?.personalityTraits ? (
                        ...
                        <div key={i} className="space-y-3 bg-white/5 p-6 rounded-3xl border border-white/5">
                           ...
                           <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                              <motion.div 
                                initial={{ width: 0 }}
                                animate={{ width: `${score}%` }}
                                className={`h-full ${i % 2 === 0 ? 'bg-cyan-500' : 'bg-purple-500'}`} 
                              />
                           </div>
                        </div>
                      ))
                      ...
                      [
                        { label: 'Technical Maturity', score: 88, color: 'bg-cyan-500' },
                        { label: 'Strategic Alignment', score: 62, color: 'bg-purple-500' },
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {[
                        ...
                        <div key={i} className="bg-white border border-gray-100 rounded-[3rem] p-8 flex items-center gap-8 group hover:border-gray-900 transition-all cursor-pointer">
                           <div className="w-16 h-16 bg-gray-50 rounded-2xl flex items-center justify-center text-gray-400 group-hover:text-cyan-600 group-hover:bg-cyan-50 transition-all">
                              <obj.icon className="w-8 h-8" />
                           </div>
                           <div className="flex-1">
                              <h4 className="font-black text-gray-900 group-hover:text-cyan-600 transition-colors">{obj.title}</h4>
