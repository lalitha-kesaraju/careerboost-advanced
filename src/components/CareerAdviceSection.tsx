...
export function CareerAdviceSection({ data, onNavigate, onDataUpdate }: CareerAdviceSectionProps) {
  ...
  if (isLimited && !careerAdvice) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center text-center p-10 bg-white rounded-[3rem] border border-blue-100 shadow-2xl">
         ...
         <p className="text-gray-500 italic serif text-lg max-w-md mb-8">
            The <span className="text-blue-700 font-bold uppercase tracking-tight">{userData?.tier}</span> plan includes {userData?.tier === 'basic' ? '1' : '4'} career strategy session. Upgrade to Premium for unlimted elite guidance.
         </p>
         <button 
           onClick={() => onNavigate('settings')}
           className="px-10 py-5 bg-blue-700 text-white rounded-2xl font-black text-lg shadow-xl shadow-blue-100 hover:bg-blue-800 transition-all flex items-center gap-3"
         >
    ...
    return (
      <div className="h-screen flex flex-col items-center justify-center space-y-6">
         <motion.div 
            ...
            className="w-16 h-16 border-4 border-blue-700 border-t-transparent rounded-full shadow-2xl"
         />
    ...
    return (
      <div className="py-20 text-center space-y-6">
         ...
         <button 
           ...
           className="px-8 py-3 bg-blue-700 text-white rounded-xl font-bold shadow-lg shadow-blue-100 transition-all hover:-translate-y-1"
         >
            Go to Analysis
         </button>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex items-center gap-6">
           <button 
             ...
             className="w-12 h-12 bg-white border border-gray-100 rounded-xl flex items-center justify-center text-gray-400 hover:text-blue-700 transition-colors"
           >
              <ArrowLeft className="w-5 h-5" />
           </button>
           <div className={`p-4 rounded-2xl ${isConfirmed ? 'bg-emerald-50 text-emerald-600' : 'bg-blue-50 text-blue-700'}`}>
           ...
           <div>
              ...
              <p className="text-gray-500 italic serif text-lg opacity-80">Path: <span className="text-blue-700 font-bold uppercase tracking-tight">{targetRole}</span></p>
           </div>
        </div>
          ...
          <div className="flex bg-white p-1.5 rounded-2xl border border-gray-100 shadow-sm">
             {[5, 10, 15].map(yr => (
               <button 
                 ...
                 className={`px-6 py-2.5 rounded-xl font-bold text-sm transition-all ${timeFilter === yr ? 'bg-blue-700 text-white shadow-lg shadow-blue-100' : 'text-gray-400 hover:text-gray-600'}`}
               >
                  {yr} Years
               </button>
             ))}
              ...
              <div className="relative z-10 space-y-6">
                 ...
                 <div className="grid grid-cols-1 gap-4">
                    <div className="p-5 bg-blue-50 rounded-2xl border border-blue-100">
                       <p className="text-[10px] text-blue-700 font-black uppercase tracking-widest mb-1">Market Benchmark Salary</p>
                       <p className="text-xl font-black text-blue-950 tracking-tight">{projection?.salaryInfo?.current}</p>
                    </div>
                    <div className="p-5 bg-blue-50 rounded-2xl border border-blue-100">
                       <p className="text-[10px] text-blue-700 font-black uppercase tracking-widest mb-1">5yr Ceiling Extension</p>
                       <p className="text-xl font-black text-blue-950 tracking-tight">{projection?.salaryInfo?.projection5yr}</p>
                    </div>
                 </div>
              </div>
           </div>
                 ...
                 >
                    <div className="flex flex-col md:flex-row justify-between items-start gap-8">
                       <div className="space-y-6 flex-1">
                          <div className="flex items-center gap-4">
                             <div className="w-14 h-14 bg-blue-700 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-blue-100">
                             ...
                             <div>
                                ...
                                <p className="text-blue-700 font-bold uppercase tracking-widest text-sm">{activeProjection?.position}</p>
                             </div>
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                             ...
                             <div className="p-8 bg-blue-50 border border-blue-100 rounded-3xl space-y-3">
                                <Zap className="w-6 h-6 text-blue-700" />
                                <h4 className="text-xl font-black text-gray-900">Strategic Move</h4>
                                <p className="text-sm text-blue-800 font-bold leading-relaxed">{activeProjection?.advice}</p>
                             </div>
                          </div>
                       </div>
                    ...
                    <div className="pt-10 border-t border-gray-100 flex flex-col items-center">
                       ...
                       <div className="flex flex-col sm:flex-row gap-4 w-full justify-center">
                          ...
                          <button 
                             onClick={() => onNavigate('resume-analysis')}
                             className="px-10 py-5 bg-white border-2 border-gray-100 text-gray-900 rounded-2xl font-black flex items-center justify-center gap-3 hover:border-blue-700 hover:text-blue-700 transition-all"
                          >
                             Re-evaluate Path
                          </button>
                       </div>
          ...
          <div className="max-w-4xl mx-auto">
             <div className="flex flex-col md:flex-row items-center gap-10">
                <div className="w-full md:w-1/3 space-y-6">
                   <div className="w-16 h-16 bg-blue-50 rounded-[2rem] flex items-center justify-center text-blue-700">
                   ...
                   <div className="space-y-4">
                      ...
                      <button 
                         ...
                         className="w-full py-5 bg-blue-700 text-white rounded-2xl font-black flex items-center justify-center gap-3 hover:bg-blue-800 transition-all shadow-xl disabled:opacity-50"
                      >
                   ...
                   <AnimatePresence mode="wait">
                      {referral ? (
                        <motion.div 
                          ...
                          initial={{ opacity: 0, scale: 0.95 }}
                          animate={{ opacity: 1, scale: 1 }}
                          className="bg-gray-50 rounded-[2.5rem] border border-blue-100 p-8 space-y-6 relative overflow-hidden"
                        >
                           <div className="absolute top-0 right-0 p-8 opacity-5 text-blue-700 pointer-events-none">
                              <MessageSquare className="w-40 h-40" />
                           </div>
                           
                           <div className="relative z-10">
                              <div className="flex items-center gap-2 mb-4">
                                 <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-[10px] font-black uppercase tracking-widest">Strategic Template</span>
                              </div>
                              ...
                              <div className="mt-6 flex items-center gap-4">
                                 <button 
                                   ...
                                   className="flex items-center gap-2 px-6 py-3 bg-white border border-gray-100 rounded-xl text-xs font-black uppercase tracking-widest hover:border-blue-500 hover:text-blue-700 transition-all shadow-sm"
                                 >
                                    <Copy className="w-4 h-4" /> Copy Message
                                 </button>
                                 <div className="flex-1 p-4 bg-blue-50 rounded-2xl">
                                    <p className="text-[10px] text-blue-700 font-black uppercase mb-1">Coach Strategy</p>
                                    <p className="text-[11px] text-blue-950 font-bold font-mono">{referral.strategy}</p>
                                 </div>
                              </div>
                           </div>
                        </motion.div>
                      ) : (
                        <div className="h-full min-h-[300px] border-2 border-dashed border-gray-100 rounded-[2.5rem] flex flex-col items-center justify-center text-center p-10 group hover:border-blue-200 transition-all">
                           <div className="w-16 h-16 bg-gray-50 rounded-[2rem] flex items-center justify-center text-gray-300 mb-6 group-hover:bg-blue-50 group-hover:text-blue-500 transition-all">
